...
% --- 1. SETTINGS ---
filename = 'tao_55_1_vacuum_nitro_raw_data.xlsx';

% Creating the temperature array: 20, 15, 10, ..., -110 (27 steps)
temps_C = 20:-5:-110; 
N = length(temps_C);
...
xlabel('Wavelength (nm)'); ylabel('Delta (deg)'); title('Delta Spectra Overlay');
...
for i = 1:N
    ...
    if col_start + 4 <= numCols
        ...
        if sum(idx) > 10
            ...
            if i > 21, xlabel('nm'); end % Add X-labels only to the bottommost plots
...
disp('Analysis complete. 27 measurement blocks down to -110C mapped successfully.');